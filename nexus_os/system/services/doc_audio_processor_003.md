...
```json
{
    "component_id": "caching_service_003",
    "enabled": true,
    "max_workers": 4,
    "timeout": 30.0,
    "retry_attempts": 3,
...
```bash
pytest tests/unit/test_caching_service_003.py
```

### Integration Tests

```bash
pytest tests/integration/test_caching_service_003.py
```

### Performance Tests

```bash
pytest tests/performance/test_caching_service_003.py
```
...
kind: Deployment
metadata:
  name: caching_service-003
spec:
  replicas: 3
  selector:
    matchLabels:
      app: caching_service-003
  template:
    metadata:
      labels:
        app: caching_service-003
