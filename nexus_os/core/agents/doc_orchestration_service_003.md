...
```json
{
    "component_id": "data_processor_003",
    "enabled": true,
    "max_workers": 4,
    "timeout": 30.0,
    "retry_attempts": 3,
...
```bash
pytest tests/unit/test_data_processor_003.py
```

### Integration Tests

```bash
pytest tests/integration/test_data_processor_003.py
```

### Performance Tests

```bash
pytest tests/performance/test_data_processor_003.py
```
...
kind: Deployment
metadata:
  name: data_processor-003
spec:
  replicas: 3
  selector:
    matchLabels:
      app: data_processor-003
  template:
    metadata:
      labels:
        app: data_processor-003
