...
```json
{
    "component_id": "routing_service_002",
    "enabled": true,
    "max_workers": 4,
    "timeout": 30.0,
    "retry_attempts": 3,
...
```bash
pytest tests/unit/test_routing_service_002.py
```

### Integration Tests

```bash
pytest tests/integration/test_routing_service_002.py
```

### Performance Tests

```bash
pytest tests/performance/test_routing_service_002.py
```
...
kind: Deployment
metadata:
  name: routing_service-002
spec:
  replicas: 3
  selector:
    matchLabels:
      app: routing_service-002
  template:
    metadata:
      labels:
        app: routing_service-002
