...
```json
{
    "component_id": "nlp_processor_000",
    "enabled": true,
    "max_workers": 4,
    "timeout": 30.0,
    "retry_attempts": 3,
...
```bash
pytest tests/unit/test_nlp_processor_000.py
```

### Integration Tests

```bash
pytest tests/integration/test_nlp_processor_000.py
```

### Performance Tests

```bash
pytest tests/performance/test_nlp_processor_000.py
```
...
kind: Deployment
metadata:
  name: nlp_processor-000
spec:
  replicas: 3
  selector:
    matchLabels:
      app: nlp_processor-000
  template:
    metadata:
      labels:
        app: nlp_processor-000
