...
```json
{
    "component_id": "routing_service_001",
    "enabled": true,
    "max_workers": 4,
    "timeout": 30.0,
    "retry_attempts": 3,
...
```bash
pytest tests/unit/test_routing_service_001.py
```

### Integration Tests

```bash
pytest tests/integration/test_routing_service_001.py
```

### Performance Tests

```bash
pytest tests/performance/test_routing_service_001.py
```
...
kind: Deployment
metadata:
  name: routing_service-001
spec:
  replicas: 3
  selector:
    matchLabels:
      app: routing_service-001
  template:
    metadata:
      labels:
        app: routing_service-001
